...
// OYTIS: TRX!
//#define CANDEV_KEA_MAX_FILTERS_RX 2
...
static int _init(candev_t *dev) {
    ...
    kea_dev->mode = CANOPT_STATE_SLEEP;
...
static void set_bittiming(candev_t *dev) {
    MSCAN->CANBTR0 = (dev->bittiming.brp & 0x3F) | (dev->bittiming.sjw << 6);
    MSCAN->CANBTR1 = (dev->bittiming.prop_seg + dev->bittiming.phase_seg1) | (dev->bittiming.phase_seg2 << 4);
}
        ...
        if (tflg) {
            for (int i = 0; i < 3; ++i) {
              if((tflg & (1 << i)) && (tx_frames[i] != NULL)) {
                 dev->event_callback(dev, CANDEV_EVENT_TX_CONFIRMATION, tx_frames[i]);
                 tx_frames[i] = NULL;
		 MSCAN->CANTIER &= ~(1 << i);
              }
            }
